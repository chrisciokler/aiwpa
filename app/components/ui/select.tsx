...
export interface SelectInputProps extends React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>, Variants {
  right?: React.ReactNode;
  left?: React.ReactNode;
}

const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, SelectInputProps>(
  ({ className, type, variant, size, children, ...props }, ref) => (
    ...
    >
      ...
      {children}
      <SelectPrimitive.Icon asChild>{props.right ? props.right : <ChevronDown className="h-4 w-4 opacity-50" />}</SelectPrimitive.Icon>
    </SelectPrimitive.Trigger>
  )
);
